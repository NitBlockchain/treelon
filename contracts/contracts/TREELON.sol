...
import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v2.3.0/contracts/utils/math/SafeMath.sol";
import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v2.3.0/contracts/utils/Address.sol";
  ...
  is ERC20, ERC20Detailed
{
  ...
  /// @notice The total number of burned TEA tokens, excluding tokens burned from a `Sell` action in the DAT.
  ...
  /// Does not affect the financial model, only the granularity of TEA.
  ...
  /// @notice The percent fee collected each time new TEA are issued expressed in basis points.
  uint public feeBasisPoints;

  /// @notice The initial fundraising goal (expressed in TEA) to start the c-org.
  ...
  /// @notice The initial number of TEA created at initialization for the beneficiary.
  ...
  /// after an investor sells his TEAs.
    ...
    authorizeTransfer(_from, _to, _amount, false)
  {
    require(state != STATE_INIT || _from == beneficiary, "ONLY_BENEFICIARY_DURING_INIT");
		(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(_amount);
		if (_isExcluded[from] && !_isExcluded[to]) {
				_transferFromExcluded(from, to, tTransferAmount);
		} else if (!_isExcluded[from] && _isExcluded[to]) {
				_transferToExcluded(from, to, tTransferAmount);
		} else if (!_isExcluded[from] && !_isExcluded[to]) {
				_transferStandard(from, to, tTransferAmount);
		} else if (_isExcluded[from] && _isExcluded[to]) {
				_transferBothExcluded(from, to, tTransferAmount);
		} else {
				_transferStandard(sender, recipient, tTransferAmount);
		}
	...
	function _transferStandard(address sender, address recipient, uint256 tAmount) private {
			...
			super._transfer(_from, _to, tTransferAmount);
...
function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
    ...
    authorizeTransfer(address(0), _to, _quantity, false)
  {
		...
		(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(_quantity);
		_reflectFee(rFee, tFee);
    super._mint(_to, tTransferAmount);
  ...
  /// @notice Calculate how many TEA tokens you would buy with the given amount of currency if `buy` was called now.
  /// @param _currencyValue How much currency to spend in order to buy TEA.
  ...
  /// @notice Purchase TEA tokens with the given amount of currency.
  /// @param _to The account to receive the TEA tokens from this purchase.
  /// @param _currencyValue How much currency to spend in order to buy TEA.
  /// @param _minTokensBought Buy at least this many TEA tokens or the transaction reverts.
    ...
    returns(uint)
  {
    ...
    if(state == STATE_RUN)
    {
      ...
      // Math: burnedSupply is capped in TEA such that the square will never overflow
      // Math worst case:
    ...
    else
    {
      ...
      // MAX * MAX_BEFORE_SQUARE
      currencyValue = _quantityToSell.mul(reserve);
      // Math: TEA blocks initReserve from being burned unless we reach the RUN state which prevents an underflow
  ...
  /// @notice Sell TEA tokens for at least the given amount of currency.
  /// @param _to The account to receive the currency from this sale.
  /// @param _quantityToSell How many TEA tokens to sell for currency value.
